...
function Addinvoice() {
  ...
  const formSubmit = async (event) => {
    ...
    if (
      ...
      // const updatedAllProd = newallprod.push(prod);
  ...
  const mainhandler = (event) => {
    ...
    switch (name) {
      ...
      case "receivername":
        ...
        break;
      case "address":
        let eaddress = value.length > 10 ? "" : "Enter Valid Address";
  ...
  const mainformSubmit = async (event) => {
    ...
    if (
      ...
      //   setUid(decode.uid);
      //   let uid=decode.uid
      //   addInvoice({ invoice,allprod,uid }).then((res) => {
